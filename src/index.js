	...
	deploy: {
		start: async (params) => {
			...
			if (config) {
				...
				const cfLambdaNames = Object.keys(cloudformation.Resources).filter(
					(name) => {
						return (
							cloudformation.Resources[name].Type ===
				...
				log(
					`"${MY_NAME}" is installed but not configured. X-Ray tracing not added.`,
				);
